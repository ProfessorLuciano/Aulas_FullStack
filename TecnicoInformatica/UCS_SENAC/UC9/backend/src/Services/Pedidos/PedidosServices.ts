...
class PedidosServices {
    ...
    async listarPedidosCliente({id}: ListarProdutos){
        const resposta = await prismaClient.pedido.findMany({
            where: {
                id_cliente: id
            },
    ...
    async listarItensPedido({ id }: ListarProdutos) {
    ...
    async alterarStatusPedido({ n_pedido, status }: AlterarStatusPedido) {
    ...
    async realizarPedidoBalcao({ id, draft, aceito, status, valor_total }: FinalizarPedidosBalcao) {
        await prismaClient.pedido.update({
            where: {
                id: id
            },
    ...
    async excluirPedido({ id }: ListarProdutos) {
        await prismaClient.pedido.delete({
            where: {
                id: id
            }
        })
        return { dados: 'Pedido Excluido com Sucesso' }
    }
